@BR_515_Hierarchic_escalated_tickets
#Automation - BR-515 - Hierarchic escalated tickets[s]
...
Feature: BR-515 - Hierarchic escalated tickets[s]
  ...
  Scenario: BR-515 - Hierarchic escalated tickets[s]
    ...
    And user validates hierarchic escalation level as "Escalation manager"
    When user clicks on work order tab
    And user validates child WorkOrder availability
    And user opens linked work order ticket
