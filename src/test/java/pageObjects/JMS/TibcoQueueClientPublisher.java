...
public class TibcoQueueClientPublisher{
    ...
    public void createTicket() throws Exception {
        ...
        {
            try {
                content_create = new String(Files.readAllBytes(file.toPath()));
                content_create = content_create.replace("CommonString", generatedString);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
...
//                "            <ns0:NMSGUID>HarvTest5</ns0:NMSGUID>" +
//                "            <ns0:AlarmGUID>HarvTest5</ns0:AlarmGUID>" +
//                "            <ns0:AlarmNumber>HarvTest5</ns0:AlarmNumber>" +
...
//                "            <ns0:JobID>HPE Common Temip HarvTest4</ns0:JobID>" +
...
//                "</ns0:BAOInputRequest>";
        ...
        System.out.println("Sending message");
            queueUtil.sendMessage(content_create, tibcoQueue);
            System.out.println(content_create);
        ...
        jdbcSQLServerConnection.connectToDatabaseAndGetOpTicket(generatedString);
    }

    public void clearAlarm() throws Exception{
        String projectPath = System.getProperty("user.dir");
        String body = projectPath + "\\src\\test\\resources\\XML Stubs\\HelixClear.xml.tld";
        File file = new File(body);
        String content_clear = "";

        {
            try {
                content_clear = new String(Files.readAllBytes(file.toPath()));
                content_clear = content_clear.replace("ticketID", CommonUtils.HelixOPID_GeneratedFromStub);
                content_clear = content_clear.replace("CommonString", generatedString);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        ...
        queueUtil.sendMessage(content_clear, tibcoQueue);
        System.out.println(content_clear);
